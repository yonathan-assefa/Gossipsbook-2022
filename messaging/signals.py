...
import json
from api.serializers.GossipSerializers import CommentRetrieveSerializer, ReplyRetrieveSerializer
from api.serializers.UserSerializers import UserRetrieveSerializer
...
def create_notification_for_true(sender, instance, action, **kwargs):
    ...
    if action == "pre_add":
        ...
        sent_user = sender.objects.last().user
        data = UserRetrieveSerializer(sent_user).data
        data["type"] = "Voted-True"
        message = f"{sent_user} Has voted True in your Gossip..."
        obj = Notifications.objects.create(user=user, message=message, notification_type="VT",
                                            data=data)
        return obj
...
def create_notification_for_true(sender, instance, action, **kwargs):
    if action == "pre_add":
        ...
        sent_user = sender.objects.last().user
        data = UserRetrieveSerializer(sent_user).data
        data["type"] = "Voted-False"

        message = f"{sent_user} Has voted False in your Gossip..."
        obj = Notifications.objects.create(user=user, message=message, data=data,
                                            notification_type="VF")
        return obj
...
def create_notification_for_replying(sender, instance, created, **kwargs):
    if created:
        ...
        data = ReplyRetrieveSerializer(instance).data
        data["type"] = "Comment-Reply"

        obj = Notifications.objects.create(user=comment_user, message=msg, data=data, 
...
def create_notification_for_commenting(sender, instance, created, **kwargs):
    if created:
        ...
        msg = f"{comment_user.username} Commented on your Gossip..."
        data = CommentRetrieveSerializer(instance).data
        data["type"] = "comment"
        obj = Notifications.objects.create(user=user, message=msg, 
                                notification_type="GC", data=data)
        return obj
